...
gwas_variables = ['subjID', 'sex', 'age', 'bmi', 'sbp', 'race', 'dm_trial',
                  'PC1', 'PC2', 'PC3', 'PC4', 'PC5',
                  'sfa_pct', 'pufa_pct', 'tot_fat_pct', 'carb', 'f2c',
                  'sfa_pct_resid', 'sbp_resid']
phenotypes = ['sfa_bmi', 'f2c_bmi', 'sfa_sbp', 'f2c_sbp', 'sfa_sbpR',
              'sfaR_sbpR']

whi_metadata_nomeds = whi_metadata.query('ht_med == False').copy()
whi_metadata_nomeds["sbp_resid"] = smf.ols('sbp ~ age + sex + bmi', 
...
gwas_phenos = (whi_metadata_nomeds
               .query('ht_med == False & dm_trial == False')
...
gwas_phenos.query('race == "white"').to_csv(
    "../data/processed/whi/whi_white_gwas_phenos_sbp.txt", sep=" ", na_rep="NA", 
    index=False)
gwas_phenos.query('race == "black"').to_csv(
    "../data/processed/whi/whi_black_gwas_phenos_sbp.txt", sep=" ", na_rep="NA", 
    index=False)
gwas_phenos.query('race == "hispanic"').to_csv(
    "../data/processed/whi/whi_hispanic_gwas_phenos_sbp.txt", sep=" ", na_rep="NA", 
    index=False)
