...
whi_scores <- parse_scorefile("../data/processed/sfa_ldl/whi_scores.sscore") %>%
...
```{r meta-white-plots}
white_meta_qq <- ggdraw() + draw_image("../data/processed/sfa_ldl/white_meta_qq.png")
white_meta_man <- ggdraw() + draw_image("../data/processed/sfa_ldl/white_meta_manhattan.png")
...
nonwhite_meta_qq <- ggdraw() + draw_image("../data/processed/sfa_ldl/nonwhite_meta_qq.png")
nonwhite_meta_man <- ggdraw() + draw_image("../data/processed/sfa_ldl/nonwhite_meta_manhattan.png")
...
```{r meta-all-plots}
all_meta_qq <- ggdraw() + draw_image("../data/processed/sfa_ldl/all_meta_qq.png")
all_meta_man <- ggdraw() + draw_image("../data/processed/sfa_ldl/all_meta_manhattan.png")
...
phenos_dm_long <- bind_rows(list(intervention=phenos_whi_intervention,
                                 control=phenos_whi_control), .id="arm")
whi_dm_long <- inner_join(whi_scores, phenos_dm_long, by="subjID") %>%
  mutate(score_bucket=cut(score, breaks=quantile(score, seq(0, 1, length.out=5)), 
...
by_assignment_plt <- ggplot(data=whi_dm_long_plt_data, aes(x=score_bucket)) +
  geom_boxplot(data=whi_dm_long, aes(y=delta_ldl), alpha=0.2) +
